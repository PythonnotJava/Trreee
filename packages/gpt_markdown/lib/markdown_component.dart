...
/// 安全的 LaTeX 渲染 widget。
///
/// 捕获 flutter_math_fork 解析/布局阶段的所有异常 (FormatException 等)，
/// 并用 ClipRect + OverflowBox 消除 RenderLine 亚像素溢出。
class _SafeLatexWidget extends StatelessWidget {
  final String tex;
  final String mathText;
  final TextStyle textStyle;
  final GptMarkdownConfig config;
  final String Function(String) workaround;
  final bool inline;

  const _SafeLatexWidget({
    required this.tex,
  ...
  Widget build(BuildContext context) {
    ...
    try {
      mathWidget = Math.tex(
        ...
        onErrorFallback: (err) => _fallbackText(context),
      );
    } catch (_) {
      // 捕获 FormatException / RangeError 等解析阶段异常
      return _fallbackText(context);
    }

    // 用 SingleChildScrollView 包裹：
    // 1. 给 Math widget 无限宽约束，防止 RenderLine 被压缩后触发亚像素溢出
    // 2. 如果公式确实比容器宽，允许水平滚动而非报错
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      physics: const ClampingScrollPhysics(),
      child: SelectableAdapter(selectedText: tex, child: mathWidget),
    );
  }
